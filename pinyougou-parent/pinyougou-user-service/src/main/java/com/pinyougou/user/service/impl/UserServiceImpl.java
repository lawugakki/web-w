...
public class UserServiceImpl implements UserService {
	...
	public void add(TbUser user) {
		user.setCreated(new Date());//注册时间为当前时间
		user.setUpdated(new Date());//修改时间也为当前时间
		user.setSourceType("1");//默认注册来源为PC
		
		String password = DigestUtils.md5Hex(user.getPassword());//将用户穿过来的密码进行md5 加密
