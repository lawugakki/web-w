...
public class GoodsController {
	...
	@Autowired Destination topicPageDeleteDestination;
	/**
	 * 批量删除
	 * @param ids
	 * @return
	...
	//@Reference(timeout=100000)  //注入search service
	...
	public Result updateStatus(Long[] ids, String status) {
		try {
			...
			if(status.equals("1")){//如果是审核通过
				...
				List<TbItem> itemList = goodsService.findItemListByGoodsIListAndStatus(ids, status);
				/*for (TbItem tbItem : itemList) {
					System.out.println(tbItem.toString());
				}*/
				/*if(itemList.size()>0){
					final  String jsonString  = JSON.toJSONString(itemList);//把list集合转换为json对象
